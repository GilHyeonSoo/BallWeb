...
// ---------------------------------------------------------------------------
// [Shader Sources & Geometry Classes - 기존과 동일]
// ---------------------------------------------------------------------------
...
class InfiniteGridMenu {
  ...
  // [추가] 활성 아이템 숨김/표시 설정 메서드
  setHideActiveItem(hide) {
    this.hideActiveItem = hide;
  }
  ...
  #initTexture() {
    ...
    Promise.all(this.items.map(item => new Promise(resolve => {
      ...
      images.forEach((img, i) => {
        ...
        // [수정됨] 중앙 확대 크롭 (Center Crop) 로직
        const shortestSide = Math.min(img.width, img.height);
        const sx = (img.width - shortestSide) / 2;
        const sy = (img.height - shortestSide) / 2;
        ctx.drawImage(img, sx, sy, shortestSide, shortestSide, x, y, cellSize, cellSize);
      });
  ...
  #animate(deltaTime) {
    ...
    positions.forEach((p, ndx) => {
      ...
      // [추가] 비디오 재생 중일 때, 활성화된(가장 가까운) 3D 원을 숨김 (크기 0)
      if (this.hideActiveItem && ndx === this.nearestVertexIndex) {
         s = 0;
      }
  ...
  #findNearestVertexIndex() {
    ...
    this.nearestVertexIndex = nearestVertexIndex; // [추가] 인스턴스 변수에 저장 (render에서 사용 위해)
...
export default function InfiniteMenu({ items = [] }) {
  ...
  useEffect(() => {
    ...
    if (canvas) {
      ...
      sketchRef.current = new InfiniteGridMenu(canvas, menuItems, handleActiveItem, setIsMoving, sk => sk.run());
    }
  ...
  useEffect(() => {
    ...
    if (isMoving) {
      setShowInfo(false);
      setIsExpanded(false); // [추가] 이동하면 확장 상태 닫기
      if(sketchRef.current) sketchRef.current.setHideActiveItem(false);
    } else {
      timer = setTimeout(() => {
        setShowInfo(true);
        if(sketchRef.current) sketchRef.current.setHideActiveItem(true);
      }, 500);
    }
  ...
  // 링크 이동 (원한다면 별도 버튼으로 분리 가능)
  const handleLinkClick = () => {
    if (activeItem?.link) window.open(activeItem.link, '_blank');
  };

  const videoUrl = (!isMoving && showInfo && activeItem?.link) ? getAutoPlayUrl(activeItem.link) : null;
  const details = activeItem?.details || {};
  
  // 소개글 처리: 확장되지 않았을 때는 100자로 자르고, 확장되면 전체 보여주기
  const introText = details.intro || "소개글이 없습니다.";
  const displayIntro = isExpanded ? introText : (introText.slice(0, 100) + (introText.length > 100 ? "..." : ""));
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      {/* 1. 중앙 비디오 */}
      {videoUrl && (
        <div className="video-overlay-container animate-video-fade-in">
          <iframe
            src={videoUrl}
            title="Active Video"
            width="100%"
      ...
      {activeItem && showInfo && (
        <>
          {/* 왼쪽: 프로필 정보 (확장 시 흐릿하게 숨기거나 그대로 유지) */}
          <div className={`info-panel left animate-slide-in-left ${isExpanded ? 'dimmed' : ''}`}>
            <h2 className="info-title">{activeItem.title}</h2>
            <div className="info-grid">
              <div className="info-item"><span className="label">품종</span> <span className="value">{details.breed}</span></div>
              <div className="info-item"><span className="label">성별</span> <span className="value">{details.sex}</span></div>
              <div className="info-item"><span className="label">생일</span> <span className="value">{details.age}</span></div>
              <div className="info-item"><span className="label">몸무게</span> <span className="value">{details.weight}</span></div>
              <div className="info-item"><span className="label">상태</span> <span className="value badge">{details.status}</span></div>
            </div>
          </div>
          ...
          <div className={`info-panel right animate-slide-in-right ${isExpanded ? 'expanded' : ''}`}>
            ...
            <div className="button-group">
              ...
              {/* 확장되었을 때만 보이는 '영상 보러가기' 버튼 (옵션) */}
              {isExpanded && (
                <button onClick={handleLinkClick} className="youtube-btn">
                  유튜브로 이동 &#x2197;
                </button>
              )}
            </div>
